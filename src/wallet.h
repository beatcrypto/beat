...
// 16789 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6666 = 16789;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BEAT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBEAT send
enum ZerocoinSpendStatus {
    ZBEAT_SPEND_OKAY = 0,                            // No error
    ZBEAT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBEAT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBEAT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBEAT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBEAT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBEAT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBEAT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBEAT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBEAT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBEAT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBEAT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBEAT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBEAT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBEAT_SPENT_USED_ZBEAT = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBeatCoinAutoBackups(bool fEnabled)
    {
