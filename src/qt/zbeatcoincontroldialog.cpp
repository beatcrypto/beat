...
#include "ui_zbeatcoincontroldialog.h"
...
std::list<std::string> ZBeatCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBeatCoinControlDialog::listMints;

ZBeatCoinControlDialog::ZBeatCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBeatCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBeatCoinControlDialog::~ZBeatCoinControlDialog()
{
    delete ui;
}

void ZBeatCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBeatCoinControlDialog::updateList()
{
...
void ZBeatCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBeatCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBeatCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBeatCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBeatCoinControlDialog::GetSelectedMints()
{
...
void ZBeatCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
