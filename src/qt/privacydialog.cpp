...
#include "zbeatcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBEAT ought to be enough for anybody." - Bill Gates, 2017
    ui->zBEATpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBEATSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBEAT->setEnabled(false);
        ui->pushButtonMintzBEAT->setToolTip(tr("zBEAT is currently disabled due to maintenance."));

        ui->pushButtonSpendzBEAT->setEnabled(false);
        ui->pushButtonSpendzBEAT->setToolTip(tr("zBEAT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBEATpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBEAT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBEAT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBEAT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBEAT in ") +
...
void PrivacyDialog::on_pushButtonSpendzBEAT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBEAT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBEAT
        sendzBEAT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBEAT
    sendzBEAT();
}

void PrivacyDialog::on_pushButtonZBeatCoinControl_clicked()
{
    ZBeatCoinControlDialog* zBeatCoinControl = new ZBeatCoinControlDialog(this);
    zBeatCoinControl->setModel(walletModel);
    zBeatCoinControl->exec();
}

void PrivacyDialog::setZBeatCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBeatCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBEAT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BeatCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBEATpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBEAT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BEAT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBEAT</b>";
    ...
    // use mints from zBeatCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBeatCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBeatCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbeatcoin selector in case it was used
    ZBeatCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBeatCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BeatCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBeatCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBEAT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBEAT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBEAT "));
    ui->labelzBEATAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBEATSyncStatus->setVisible(fShow);
}
